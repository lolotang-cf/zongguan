...
function buildBusinessSeed() {
  ...
  // 权限地基：给所有明细列表补 center（确定性轮询，可复现）
  enrichCenters(raw);
  return raw;
}

/**
 * 权限地基：给业务集合的"明细列表"补充 center 字段。
 * 已是"按中心汇总"且自带 center 的对象（如 establishment.centerSummary）保留原值；
 * 其余明细对象按中心池轮询分配，保证数据范围隔离可演示。
 */
const CENTER_POOL = ['上海', '太原', '长沙', '南昌', '南宁', '郑州', '沈阳', '深圳'];
const DETAIL_KEYS = ['detail', 'list', 'recent', 'byCenter', 'centerSummary', 'projectDetail',
  'centerOverview', 'projectSummary', 'inventory', 'records', 'devices', 'questions', 'exams',
  'courses', 'trend', 'todo', 'dailyData', 'rpoChannels', 'weeklyProcess', 'deptSummary',
  'channels', 'activities', 'centerUsage', 'platformContent', 'hotTopics', 'alerts', 'drillPlan',
  'applications', 'matrix', 'categoryTree', 'learningPaths', 'tagCloud', 'levelDist', 'typeDist',
  'statusDist', 'centerDist', 'starDist', 'reasons', 'dist', 'monthly', 'compare', 'centerAvg',
  'details', 'items'];
let _ci = 0;
function pickCenter() { const c = CENTER_POOL[_ci % CENTER_POOL.length]; _ci++; return c; }

function enrichCenters(biz) {
  // 1) 顶层即为数组的集合（contracts / hrProcesses / supplies / regulation ...）
  for (const name of Object.keys(biz)) {
    const v = biz[name];
    if (Array.isArray(v) && v.length && v[0] && typeof v[0] === 'object') {
      v.forEach(it => { if (it && typeof it === 'object' && !('center' in it)) it.center = pickCenter(); });
    }
  }
  // 2) 对象集合内的明细数组
  for (const name of Object.keys(biz)) {
    ...
    if (v && typeof v === 'object' && !Array.isArray(v)) {
      DETAIL_KEYS.forEach(key => {
        ...
        if (Array.isArray(arr) && arr.length && arr[0] && typeof arr[0] === 'object') {
          arr.forEach(it => { if (it && typeof it === 'object' && !('center' in it)) it.center = pickCenter(); });
        }
      });
    }
  }
...
/**
 * 预警引擎：扫描业务数据生成预警列表。
 * 覆盖：合同到期(daysLeft)、试用期到期(daysLeft)。招聘平台账号到期暂无数据（需求模块三），留 TODO。
 */
function computeAlerts(biz) {
  const out = [];
  const now = Date.now();
  // 合同到期
  const contracts = Array.isArray(biz.contracts) ? biz.contracts : [];
  contracts.forEach(c => {
    const dl = Number(c.daysLeft);
    if (!isNaN(dl) && dl <= 90) {
      const level = dl <= 30 ? 'high' : dl <= 60 ? 'mid' : 'low';
      out.push({
        id: 'AL-C-' + c.id, level, module: 'hr', type: '合同到期',
        title: `${c.name}（${c.department}/${c.center}）合同即将到期`,
        desc: `合同类型：${c.type}；到期日：${c.endDate}；剩余 ${dl} 天；续签状态：${c.renewStatus || '待处理'}`,
        status: 'open', due: c.endDate, center: c.center, createdAt: now
      });
    }
  });
  // 试用期到期
  const prob = (biz.probation && Array.isArray(biz.probation.list)) ? biz.probation.list : [];
  prob.forEach(p => {
    const dl = Number(p.daysLeft);
    if (!isNaN(dl) && dl <= 15) {
      const level = dl <= 7 ? 'high' : 'mid';
      out.push({
        id: 'AL-P-' + p.id, level, module: 'hr', type: '试用期到期',
        title: `${p.name}（${p.dept}/${p.center}）试用期即将到期`,
        desc: `入职日：${p.joinDate}；到期日：${p.expireDate}；剩余 ${dl} 天`,
        status: 'open', due: p.expireDate, center: p.center, createdAt: now
      });
    }
  });
  return out;
...
/** 用户表（用于真实登录 + RBAC + 数据范围 center）。密码为初始密码，首次登录建议修改。 */
function buildUsers() {
  ...
  return [
    { id: 'U001', username: 'admin',   name: '系统管理员', role: 'super_admin',  dept: '综管部', title: '负责人',     center: '*',     passwordHash: hash, mustChangePwd: false },
    { id: 'U002', username: 'zhao',    name: '赵主管', role: 'admin_zg',     dept: '综管部', title: '综管主管',   center: '*',     passwordHash: hash, mustChangePwd: true },
    { id: 'U003', username: 'recruit', name: '招聘主管', role: 'recruit_lead', dept: '招聘', title: '招聘主管', center: '上海',   passwordHash: hash, mustChangePwd: true },
    { id: 'U004', username: 'train',   name: '培训主管', role: 'train_lead',   dept: '培训', title: '培训主管', center: '太原',   passwordHash: hash, mustChangePwd: true },
    { id: 'U005', username: 'hr',      name: '人事专员', role: 'hr_clerk',     dept: '人事', title: '人事专员', center: '长沙',   passwordHash: hash, mustChangePwd: true },
    { id: 'U006', username: 'adminc',  name: '行政专员', role: 'admin_clerk',  dept: '行政', title: '行政专员', center: '南昌',   passwordHash: hash, mustChangePwd: true },
    { id: 'U007', username: 'pr',      name: '公关专员', role: 'pr_clerk',     dept: '公关', title: '公关专员', center: '南宁',   passwordHash: hash, mustChangePwd: true },
    { id: 'U008', username: 'staff',   name: '普通员工', role: 'staff',        dept: '全员', title: '员工',     center: '郑州',   passwordHash: hash, mustChangePwd: true }
  ];
}
