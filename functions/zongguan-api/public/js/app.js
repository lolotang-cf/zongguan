...
const NAV_CONFIG = [
  ...
  { group: '人员台账', children: [
    { subgroup: '招聘管理', items: [
      ...
      { id: 'recruit-backoffice', icon: '🏢', name: '二线职能达成', title: '二线职能需求达成看板' },
      { id: 'recruit-rpo', icon: '🎓', name: 'RPO校招看板', title: 'RPO校招看板' }
    ]},
    { subgroup: '人事管理', items: [
      ...
      { id: 'hr-probation', icon: '⏰', name: '试用期到期预警', title: '试用期到期预警' },
      { id: 'hr-weekly', icon: '📰', name: '人力周报', title: '人力周报' },
      { id: 'hr-labor', icon: '⚖️', name: '劳动关系管理', title: '劳动关系管理' },
      { id: 'hr-policy', icon: '📘', name: '人事制度库', title: '人事制度知识库' }
    ]},
    { subgroup: '培训管理', items: [
      { id: 'train-talent', icon: '👥', name: '人才库看板', title: '人才库数据看板' },
      { id: 'train-new', icon: '🎓', name: '新培库看板', title: '新培库数据看板' },
      { id: 'train-instructor', icon: '👨‍🏫', name: '讲师库', title: '讲师库数据看板' },
      { id: 'train-course', icon: '📖', name: '课程库', title: '课程库数据看板' },
      { id: 'train-cadre-loss', icon: '👔', name: '干部流失分析', title: '干部流失分析' },
      { id: 'train-est-lib', icon: '📊', name: '编制库', title: '编制库趋势看板' },
      { id: 'train-knowledge', icon: '📚', name: '培训知识库', title: '培训知识库' },
      { id: 'train-exam', icon: '📝', name: '题库与考试', title: '岗前培训考试题库与在线考试' },
      { id: 'train-material', icon: '📦', name: '培训物料管理', title: '培训物料库存管理' }
    ]}
  ]},
  { group: '物品台账', items: [
    ...
    { id: 'admin-manual', icon: '📖', name: '行政操作手册', title: '行政操作手册知识库' }
  ]},
  { group: '事务台账', items: [
  ...
  { group: '公关台账', items: [
    { id: 'pr-opinion', icon: '📢', name: '舆情监控', title: '舆情监控看板' },
    { id: 'pr-crisis', icon: '🚨', name: '危机预案库', title: '危机预案知识库' },
    { id: 'pr-media', icon: '📡', name: '媒体资源库', title: '媒体资源库' }
  ]},
  { group: '系统管理', items: [
...
window.showWorkplaceDetail = function(d) {
  ...
  showDetailModal('职场详情 - ' + d.id,
    ...
    + '</div>'
    + '<div class="detail-section" style="margin-top:16px"><div class="detail-section-title">📊 工位使用</div>'
    ...
    + '<div class="detail-actions"><button class="btn btn-primary" onclick="closeDetailModal()">✏️ 编辑</button>'
...
window.showAssetDetail = function(a) {
  showDetailModal('资产详情 - ' + a.id,
    ...
    + '</div>'
    + '<div class="detail-section" style="margin-top:16px"><div class="detail-section-title">📝 资产档案</div>'
...
window.showAssetTransfer = function(asset) {
  ...
  showDetailModal('🔄 固资转出' + (assetName ? ' - ' + assetName : ''),
    ...
    + '<div style="font-size:13px;color:#d48806;font-weight:600;margin-bottom:4px">⚠️ 转出须知</div>'
...
window.showSupplyRequisition = function(i) {
  showDetailModal('领用 - ' + i.name,
    ...
    + '</div>'
    + '<div class="detail-section"><div class="detail-section-title">📥 领用申请</div>'
...
window.showCaseDetail = function(r) {
  showDetailModal('劳动案件 - ' + r.id,
    ...
    + '</div>'
    + '<div class="detail-section" style="margin-top:16px"><div class="detail-section-title">📋 处理时间线</div>'
    + '<div class="detail-timeline">'
...
window.showStaffingDetail = function(s) {
  showDetailModal('编制详情 - ' + s.dept,
    ...
    + '</div>'
    + '<div class="detail-actions"><button class="btn btn-primary" onclick="closeDetailModal()">✏️ 调整编制</button>'
    + '<button class="btn btn-outline" onclick="closeDetailModal()">📋 导出</button>'
    + '<button class="btn btn-outline" onclick="closeDetailModal()" style="margin-left:auto">关闭</button></div>'
  );
...
async function loadDashboard() {
  ...
  // 五模块核心指标红绿灯
  const lightData = [
    { module: '招聘管理', icon: '🎯', metric: '年度达成率', target: 80, actual: data.recruit.rate },
    { module: '培训管理', icon: '🎓', metric: '新培转化率', target: 85, actual: data.train.newTrain.rate },
    { module: '人事管理', icon: '👥', metric: '编制使用率', target: 95, actual: data.hr.headcount.rate },
    { module: '行政管理', icon: '📦', metric: '预算执行率', target: 55, actual: data.admin.budget.rate },
    { module: '公关管理', icon: '📢', metric: '舆情正面率', target: 70, actual: data.pr.sentiment.positive }
  ];
  const getLight = (t, a) => a >= t ? { emoji: '🟢', color: COLORS.green, bar: 'green', text: '达标' }
    : a >= t * 0.9 ? { emoji: '🟡', color: COLORS.orange, bar: 'orange', text: '接近' }
    : { emoji: '🔴', color: COLORS.red, bar: 'red', text: '未达标' };
  ...
  renderPage(`
    ...
    <div class="chart-card" style="margin-top:8px">
      <div class="chart-title">🔄 系统数据同步状态</div>
...
async function loadSysPermission() {
  ...
  renderPage(`
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">👥</div><div class="kpi-value">${data.overview.totalRoles}</div><div class="kpi-label">角色总数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">👤</div><div class="kpi-value">${data.overview.totalUsers}</div><div class="kpi-label">系统用户数</div></div>
    ...
    <div class="row cols-2">
      ...
      <div class="chart-card">
        <div class="chart-title">📋 模块权限分配</div>
        <table class="data-table">
          <thead><tr><th>模块</th><th>查看</th><th>编辑</th><th>审批</th><th>导出</th><th>管理</th></tr></thead>
          <tbody>
            ${data.modules.map(m => `<tr><td><strong>${m.name}</strong></td><td>${m.view ? '✅' : '❌'}</td><td>${m.edit ? '✅' : '❌'}</td><td>${m.approve ? '✅' : '❌'}</td><td>${m.export ? '✅' : '❌'}</td><td>${m.admin ? '✅' : '❌'}</td></tr>`).join('')}
          </tbody>
        </table>
      </div>
    </div>
    <div class="chart-card">
      <div class="chart-title">🔔 权限申请记录</div>
      <table class="data-table">
...
async function loadTrainNew() {
  ...
  renderPage(`
    ...
    <div class="row cols-2">
      <div class="chart-card">
        <div class="chart-title">🔄 参训→签约→入组 转化漏斗</div>
            ...
            ${list.map(k => `
              <tr>
                ...
                <td>
                  ...
                  <button class="btn btn-outline" style="padding:3px 10px;font-size:12px" onclick="showConfirmModal(\'⬇️ 下载文档 - \' + rowCellText(this,1),\'<p style=&quot;margin:0;color:#666;font-size:13px&quot;>开始下载文档。</p>\',\'开始下载\',\'文档开始下载\')">下载</button>
                </td>
              </tr>
            `).join('')}
          </tbody>
  ...
  window.openCourseDetail = function(courseId) {
    ...
    document.getElementById('kbModalBody').innerHTML = `
      ...
      <div class="kb-detail-actions">
        ...
        <button class="btn btn-outline" onclick="showConfirmModal(\'⬇️ 下载文档 - \' + rowCellText(this,1),\'<p style=&quot;margin:0;color:#666;font-size:13px&quot;>开始下载文档。</p>\',\'开始下载\',\'文档开始下载\')">⬇️ 下载课件</button>
...
async function loadTrainExam() {
  ...
  renderPage(`
    ...
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">📅</div><div class="kpi-value">${monthExamCount}</div><div class="kpi-label">本月考试次数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">👥</div><div class="kpi-value">${totalCandidates}</div><div class="kpi-label">累计参考人次</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">📈</div><div class="kpi-value">${avgPassRate}%</div><div class="kpi-label">平均通过率</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.red}"></div><div class="kpi-icon">🏆</div><div class="kpi-value">${maxScore}</div><div class="kpi-label">最高分</div></div>
    </div>
    ...
    <div id="teTab-wrong" style="display:none">
      <div class="chart-card">
        <div class="chart-title">⚠️ 高频错题排行 TOP5</div>
...
async function loadHrProcess() {
  ...
  renderPage(`
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">⏳</div><div class="kpi-value">${pending}</div><div class="kpi-label">审批中数量</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">✅</div><div class="kpi-value">${passed}</div><div class="kpi-label">本月已通过</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.red}"></div><div class="kpi-icon">❌</div><div class="kpi-value">${rejected}</div><div class="kpi-label">本月已驳回</div></div>
...
async function loadHrContract() {
  ...
  renderPage(`
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.red}"></div><div class="kpi-icon">🚨</div><div class="kpi-value">${in30}</div><div class="kpi-label">30天内到期</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">⚠️</div><div class="kpi-value">${in60}</div><div class="kpi-label">30-60天到期</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">📅</div><div class="kpi-value">${in90}</div><div class="kpi-label">60-90天到期</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">📋</div><div class="kpi-value">${data.length}</div><div class="kpi-label">待续签合同总数</div></div>
    </div>
    <div class="chart-card">
...
async function loadAdminSupplies() {
  ...
  renderPage(`
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">📦</div><div class="kpi-value">${items.length}</div><div class="kpi-label">物品种类</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">📊</div><div class="kpi-value">${items.reduce((s,i)=>s+(i.stock||0),0)}</div><div class="kpi-label">库存总量</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.red}"></div><div class="kpi-icon">⚠️</div><div class="kpi-value">${items.filter(i=>i.status==='预警').length}</div><div class="kpi-label">库存预警</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">📤</div><div class="kpi-value">${(data.usage && data.usage.length) ? data.usage[data.usage.length-1].count : 0}</div><div class="kpi-label">本月领用</div></div>
    </div>
    <div class="row cols-2">
      <div class="chart-card">
...
async function loadHrTurnover() {
  ...
  renderPage(`
    <div class="kpi-grid cols-5">
      ...
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">📊</div><div class="kpi-value">${(ov.rate || 0)}%</div><div class="kpi-label">流失率</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.purple}"></div><div class="kpi-icon">📅</div><div class="kpi-value">${ov.monthTurnover || 0}</div><div class="kpi-label">本月流失</div></div>
    ...
    <div class="row cols-2">
      <div class="chart-card">
        <div class="chart-title">📈 月度人员变动趋势</div>
    ...
    <div class="row cols-2">
      ...
      <div class="chart-card">
        <div class="chart-title">📋 近期流失明细</div>
        <table class="data-table">
...
async function loadHrEstablishment() {
  ...
  renderPage(`
    <div class="kpi-grid cols-5">
      ...
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.purple}"></div><div class="kpi-icon">🎯</div><div class="kpi-value">${data.overview.recruiting}</div><div class="kpi-label">招聘中</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.cyan}"></div><div class="kpi-icon">📈</div><div class="kpi-value">${data.overview.utilizationRate}%</div><div class="kpi-label">编制使用率</div></div>
    </div>
    <div class="row cols-2">
      <div class="chart-card">
        <div class="chart-title">📊 各中心编制分布（含招聘中）</div>
        <div id="estCenterChart" style="height:320px"></div>
      </div>
      <div class="chart-card">
        <div class="chart-title">📈 编制使用率趋势</div>
...
async function loadAdminPrint() {
  ...
  renderPage(`
    ...
    <div class="chart-card">
      <div class="chart-title">📋 各中心打印量排名</div>
      <table class="data-table">
    ...
    <div class="chart-card">
      <div class="chart-title">📋 纸张消耗明细</div>
      <table class="data-table">
...
async function loadAdminKpi() {
  ...
  const data = hasModuleData ? rawData : {
    ...
    modules: [
      { name: '招聘管理', icon: '🎯', target: 80, actual: 68.1, score: 72, trend: 'up', people: 18,
        kpis: [
      ...
      { name: '培训管理', icon: '🎓', target: 85, actual: 79.2, score: 81, trend: 'up', people: 15,
        kpis: [
      ...
      { name: '人事行政', icon: '👥', target: 95, actual: 93.9, score: 90, trend: 'stable', people: 32,
        kpis: [
      ...
      { name: '公共管理', icon: '🏢', target: 70, actual: 68.0, score: 75, trend: 'down', people: 41,
        kpis: [
  ...
  renderPage(`
    <div class="section-title">🎯 综管五模块核心KPI</div>
    <div class="kpi-grid cols-5">
      ${mods.map((m, idx) => {
    ...
    <div class="row cols-2">
      <div class="chart-card">
        <div class="chart-title">📊 五模块KPI达成对比</div>
    ...
    <div class="chart-card">
      <div class="chart-title">📋 KPI考核明细（按模块）</div>
      <div class="filter-bar">
...
async function loadHrProbation() {
  ...
  renderPage(`
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.red}"></div><div class="kpi-icon">🚨</div><div class="kpi-value">${data.overview.expired}</div><div class="kpi-label">已到期未评估</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">⏰</div><div class="kpi-value">${data.overview.in7Days}</div><div class="kpi-label">7天内到期</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">📅</div><div class="kpi-value">${data.overview.in15Days}</div><div class="kpi-label">15天内到期</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">✅</div><div class="kpi-value">${data.overview.total}</div><div class="kpi-label">试用期总人数</div></div>
    </div>
    <div class="chart-card">
...
async function loadHrWeekly() {
  ...
  renderPage(`
    ...
    <div class="kpi-grid cols-5">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">➕</div><div class="kpi-value">${data.summary.onboard}</div><div class="kpi-label">本周入职</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.red}"></div><div class="kpi-icon">➖</div><div class="kpi-value">${data.summary.resign}</div><div class="kpi-label">本周离职</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">✅</div><div class="kpi-value">${data.summary.regular}</div><div class="kpi-label">本周转正</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.purple}"></div><div class="kpi-icon">🔄</div><div class="kpi-value">${data.summary.transfer}</div><div class="kpi-label">本周调岗</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">🎯</div><div class="kpi-value">${data.summary.recruitProgress}</div><div class="kpi-label">招聘完成率</div></div>
    </div>
    <div class="row cols-2">
      <div class="chart-card">
        <div class="chart-title">📊 各中心人员变动</div>
        <div id="weeklyCenterChart" style="height:300px"></div>
      </div>
      <div class="chart-card">
        <div class="chart-title">📈 近8周入职离职趋势</div>
        <div id="weeklyTrendChart" style="height:300px"></div>
      </div>
    </div>
    <div class="chart-card">
      <div class="chart-title">🔔 下周待办事项</div>
      <table class="data-table">
...
async function loadHrLabor() {
  ...
  renderPage(`
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">⚖️</div><div class="kpi-value">${data.overview.total}</div><div class="kpi-label">案件总数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">⏳</div><div class="kpi-value">${data.overview.pending}</div><div class="kpi-label">处理中</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">✅</div><div class="kpi-value">${data.overview.resolved}</div><div class="kpi-label">已结案</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.red}"></div><div class="kpi-icon">💰</div><div class="kpi-value">${data.overview.totalCompensation}万</div><div class="kpi-label">累计经济补偿</div></div>
    </div>
    <div class="row cols-2">
      <div class="chart-card">
...
async function loadTrainInstructor() {
  ...
  renderPage(`
    <div class="kpi-grid cols-5">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">👨‍🏫</div><div class="kpi-value">${data.overview.total}</div><div class="kpi-label">讲师总数</div></div>
...
async function loadTrainCourse() {
  ...
  renderPage(`
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">📖</div><div class="kpi-value">${data.overview.totalCourses}</div><div class="kpi-label">课程总数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">✅</div><div class="kpi-value">${data.overview.published}</div><div class="kpi-label">已发布</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">🔄</div><div class="kpi-value">${data.overview.updating}</div><div class="kpi-label">更新中</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.purple}"></div><div class="kpi-icon">👥</div><div class="kpi-value">${data.overview.totalLearners}</div><div class="kpi-label">累计学习人次</div></div>
    </div>
    <div class="chart-card">
      <div class="chart-title">📊 各团队课程矩阵</div>
...
async function loadTrainCadreLoss() {
  ...
  renderPage(`
    <div class="kpi-grid cols-5">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">👔</div><div class="kpi-value">${data.overview.totalCadres}</div><div class="kpi-label">干部总数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.red}"></div><div class="kpi-icon">📉</div><div class="kpi-value">${data.overview.lossCount}</div><div class="kpi-label">流失人数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">📊</div><div class="kpi-value">${data.overview.lossRate}%</div><div class="kpi-label">流失率</div></div>
    ...
    <div class="row cols-2">
      <div class="chart-card">
        <div class="chart-title">📊 后备 vs 在职干部分布</div>
        <div id="cadreDistChart" style="height:320px"></div>
      </div>
      <div class="chart-card">
        <div class="chart-title">🔍 流失原因分析</div>
...
async function loadTrainEstLib() {
  ...
  renderPage(`
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">📐</div><div class="kpi-value">${data.overview.total}</div><div class="kpi-label">编制总数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">✅</div><div class="kpi-value">${data.overview.registered}</div><div class="kpi-label">在册人数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">📋</div><div class="kpi-value">${data.overview.orders}</div><div class="kpi-label">订单总数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.purple}"></div><div class="kpi-icon">📊</div><div class="kpi-value">${data.overview.utilRate}%</div><div class="kpi-label">使用率</div></div>
    </div>
    <div class="row cols-2">
      <div class="chart-card">
        <div class="chart-title">📈 编制增长趋势（同比）</div>
        <div id="estLibTrendChart" style="height:320px"></div>
      </div>
      <div class="chart-card">
        <div class="chart-title">📊 各中心编制对比</div>
...
async function loadTrainMaterial() {
  ...
  renderPage(`
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">📦</div><div class="kpi-value">${data.overview.totalItems}</div><div class="kpi-label">物料种类</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">✅</div><div class="kpi-value">${data.overview.normalStock}</div><div class="kpi-label">库存正常</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.red}"></div><div class="kpi-icon">⚠️</div><div class="kpi-value">${data.overview.lowStock}</div><div class="kpi-label">库存预警</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.purple}"></div><div class="kpi-icon">📝</div><div class="kpi-value">${data.overview.totalRecords}</div><div class="kpi-label">本月领用记录</div></div>
    </div>
    <div class="chart-card">
    ...
    <div class="chart-card">
      <div class="chart-title">📝 近期领用记录</div>
      <table class="data-table">
...
async function loadRecruitBackoffice() {
  ...
  renderPage(`
    ...
    <div class="kpi-grid cols-5">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">📋</div><div class="kpi-value">${data.overview.totalDemand}</div><div class="kpi-label">年度需求总数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">✅</div><div class="kpi-value">${data.overview.totalOnboard}</div><div class="kpi-label">已入职人数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">📊</div><div class="kpi-value">${data.overview.achieveRate}%</div><div class="kpi-label">整体达成率</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.purple}"></div><div class="kpi-icon">🔄</div><div class="kpi-value">${data.overview.inProgress}</div><div class="kpi-label">招聘中</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.red}"></div><div class="kpi-icon">⏳</div><div class="kpi-value">${data.overview.pending}</div><div class="kpi-label">待启动</div></div>
    </div>
    <div class="row cols-2">
      <div class="chart-card">
        <div class="chart-title">📊 各职能需求达成对比</div>
        <div id="boDeptChart" style="height:300px"></div>
      </div>
      <div class="chart-card">
        <div class="chart-title">📈 月度需求vs入职趋势</div>
        <div id="boTrendChart" style="height:300px"></div>
      </div>
    </div>
    <div class="chart-card">
      <div class="chart-title">📋 二线职能需求明细</div>
      <table class="data-table">
...
async function loadRecruitRpo() {
  ...
  renderPage(`
    ...
    <div class="kpi-grid cols-5">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">🎓</div><div class="kpi-value">${data.overview.totalCandidates}</div><div class="kpi-label">总候选人数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.purple}"></div><div class="kpi-icon">📋</div><div class="kpi-value">${data.overview.interviewed}</div><div class="kpi-label">面试人数</div></div>
      ...
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">✅</div><div class="kpi-value">${data.overview.onboarded}</div><div class="kpi-label">入职人数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.red}"></div><div class="kpi-icon">📉</div><div class="kpi-value">${data.overview.conversionRate}%</div><div class="kpi-label">整体转化率</div></div>
    </div>
    <div class="row cols-2">
      <div class="chart-card">
        <div class="chart-title">📊 各渠道RPO转化漏斗</div>
        <div id="rpoFunnel" style="height:300px"></div>
      </div>
      <div class="chart-card">
        <div class="chart-title">📈 校招vs RPO对比</div>
    ...
    <div class="row cols-2">
      ...
      <div class="chart-card">
        <div class="chart-title">🔔 近期校招活动</div>
        ${data.activities.map(a => `<div class="alert-item alert-${a.status==='进行中'?'info':'success'}"><span class="alert-icon">🎓</span><div><strong>${a.name}</strong> · ${a.date}<br><span style="font-size:12px;color:var(--text-light)">${a.school} | 岗位: ${a.position} | 候选${a.candidates}人 | ${a.status}</span></div></div>`).join('')}
      </div>
    </div>
  `);
  setTimeout(() => {
...
async function loadPrCrisis() {
  ...
  renderPage(`
    <div class="filter-bar">
      ...
      <button class="btn btn-outline">➕ 新建预案</button>
    </div>
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">📋</div><div class="kpi-value">${data.overview.total}</div><div class="kpi-label">预案总数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">✅</div><div class="kpi-value">${data.overview.published}</div><div class="kpi-label">已发布</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">🔄</div><div class="kpi-value">${data.overview.drilling}</div><div class="kpi-label">演练中</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.red}"></div><div class="kpi-icon">⚠️</div><div class="kpi-value">${data.overview.expiring}</div><div class="kpi-label">即将过期</div></div>
    </div>
    <div class="chart-card">
      <div class="chart-title">📋 危机预案列表</div>
      <table class="data-table">
    ...
    <div class="row cols-2">
      <div class="chart-card">
        <div class="chart-title">📊 预案等级分布</div>
        <div id="crisisLevel" style="height:260px"></div>
      </div>
      <div class="chart-card">
        <div class="chart-title">📅 年度演练计划</div>
        <table class="data-table">
...
async function loadPrMedia() {
  ...
  renderPage(`
    <div class="filter-bar">
      ...
      <button class="btn btn-primary">🔍 搜索</button>
      <button class="btn btn-outline">➕ 新增媒体</button>
    </div>
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">📡</div><div class="kpi-value">${data.overview.totalMedia}</div><div class="kpi-label">合作媒体数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">👤</div><div class="kpi-value">${data.overview.totalContacts}</div><div class="kpi-label">记者联系人</div></div>
      ...
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.purple}"></div><div class="kpi-icon">📰</div><div class="kpi-value">${data.overview.recentReports}</div><div class="kpi-label">本月报道数</div></div>
    </div>
    <div class="chart-card">
      <div class="chart-title">📋 媒体资源明细</div>
      <table class="data-table">
    ...
    <div class="row cols-2">
      <div class="chart-card">
        <div class="chart-title">📊 媒体类型分布</div>
        <div id="mediaTypeChart" style="height:260px"></div>
      </div>
      <div class="chart-card">
        <div class="chart-title">📈 月度报道趋势</div>
...
async function loadHrPolicy() {
  ...
  renderPage(`
    <div class="filter-bar">
      <input type="text" placeholder="🔍 搜索制度名称/关键词..." style="flex:1;max-width:400px">
      ...
      <button class="btn btn-primary">搜索</button>
      <button class="btn btn-outline">➕ 上传制度</button>
    </div>
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">📚</div><div class="kpi-value">${data.overview.total}</div><div class="kpi-label">制度总数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">✅</div><div class="kpi-value">${data.overview.active}</div><div class="kpi-label">现行制度</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">🔄</div><div class="kpi-value">${data.overview.revising}</div><div class="kpi-label">修订中</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.purple}"></div><div class="kpi-icon">👁️</div><div class="kpi-value">${data.overview.totalViews}</div><div class="kpi-label">总阅读量</div></div>
    </div>
    <div class="chart-card">
      <div class="chart-title">📋 人事制度列表</div>
      <table class="data-table">
...
async function loadAdminManual() {
  ...
  renderPage(`
    <div class="filter-bar">
      <input type="text" placeholder="🔍 搜索手册/操作流程..." style="flex:1;max-width:400px">
      ...
      <button class="btn btn-primary">搜索</button>
      <button class="btn btn-outline">➕ 上传手册</button>
    </div>
    <div class="kpi-grid cols-4">
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.blue}"></div><div class="kpi-icon">📖</div><div class="kpi-value">${data.overview.total}</div><div class="kpi-label">手册总数</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.green}"></div><div class="kpi-icon">✅</div><div class="kpi-value">${data.overview.active}</div><div class="kpi-label">现行手册</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.orange}"></div><div class="kpi-icon">🔄</div><div class="kpi-value">${data.overview.updating}</div><div class="kpi-label">更新中</div></div>
      <div class="kpi-card"><div class="kpi-accent" style="background:${COLORS.purple}"></div><div class="kpi-icon">👁️</div><div class="kpi-value">${data.overview.totalViews}</div><div class="kpi-label">总阅读量</div></div>
    </div>
    <div class="chart-card">
      <div class="chart-title">📖 行政操作手册列表</div>
      <table class="data-table">
...
async function loadHrCoreKpi() {
  ...
  if (!data) return;
  renderPage(`
    <div class="section-title">📊 人事核心 KPI 指标</div>
    <div class="kpi-grid cols-4">
      ${[ {v:data.headcountRate||0,l:'编制使用率',icon:'📐',c:COLORS.blue},{v:data.contractExpire30||0,l:'30天到期合同',icon:'📋',c:COLORS.red},{v:data.probationExpire14||0,l:'14天到期试用',icon:'⏰',c:COLORS.orange},{v:data.laborRisk||0,l:'劳动关系风险',icon:'⚖️',c:COLORS.purple} ].map(k=>`<div class="kpi-card"><div class="kpi-accent" style="background:${k.c}"></div><div class="kpi-icon">${k.icon}</div><div class="kpi-value">${k.v}</div><div class="kpi-label">${k.l}</div></div>`).join('')}
    </div>`);
}
async function loadHrWeekly() {
  const data = await fetchAPI(API.hrWeekly);
  if (!data) return;
  renderPage(`<div class="section-title">📰 人力周报</div><div class="kpi-grid cols-5">${(data.summary?[data.summary]:[]).map(s=>`<div class="kpi-card"><div class="kpi-value">${s.onboard+s.resign+s.regular+s.transfer}</div><div class="kpi-label">本周变动</div></div>`).join('')}</div><div class="chart-card"><div class="chart-title">各中心周度明细</div><div id="hrWeeklyChart" style="height:320px"></div></div>`);
...
async function loadHrProbation() {
  ...
  renderPage(`
    <div class="section-title">⏰ 试用期到期预警</div>
    <div class="kpi-grid cols-4" style="margin-bottom:16px">
      <div class="kpi-card"><div class="kpi-icon">⚠️</div><div class="kpi-value">${list.length}</div><div class="kpi-label">试用期总人数</div></div>
      <div class="kpi-card"><div class="kpi-icon">🔴</div><div class="kpi-value">${list.filter(x=>x.daysLeft<=7).length}</div><div class="kpi-label">7天内到期</div></div>
      <div class="kpi-card"><div class="kpi-icon">🟡</div><div class="kpi-value">${list.filter(x=>x.daysLeft>7&&x.daysLeft<=14).length}</div><div class="kpi-label">14天内到期</div></div>
      <div class="kpi-card"><div class="kpi-icon">🟢</div><div class="kpi-value">${list.filter(x=>x.daysLeft>14).length}</div><div class="kpi-label">14天后到期</div></div>
    </div>
    <div class="chart-card">
...
async function loadAlerts() {
  ...
  renderPage(`
    <div class="section-title">⚠️ 预警中心</div>
    <div class="alert-center">${list.map(a => `
      <div class="alert-card ${lvMap[a.level]||''}">
        ...
        <div class="alert-foot"><span>📅 ${a.createdAt||'-'}</span><span>📍 ${a.center||'-'}</span></div>
